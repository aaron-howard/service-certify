...
| Sentry alert rules | New issue + error spike in `production` → email — run `npm run setup:sentry-alerts` ([SENTRY-SETUP.md](./SENTRY-SETUP.md)) | Ops — script ready; needs `SENTRY_AUTH_TOKEN` with `alerts:write` |
...
12. [ ] Sentry alert rules: `SENTRY_AUTH_TOKEN=… npm run setup:sentry-alerts` + ignore SERVICE-CERTIFY-6
