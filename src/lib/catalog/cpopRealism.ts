/** CPOP (Certified Platform Owner Professional) exam-realism rules — April 2026 blueprint. */
...
export const CPOP_DOMAIN_TARGETS = {
	Strategy: 40,
	People: 10,
	Process: 10,
	Technology: 20,
	Data: 10,
	'ServiceNow Governance': 10
} as const;

export const CPOP_BANK_SIZE = 100;

export type CpopDomain = keyof typeof CPOP_DOMAIN_TARGETS;

export function domainForOrder(order: number): CpopDomain {
...
export type CpopQuestionRow = RealismQuestionRow;
...
export function validateCpopQuestion(q: CpopQuestionRow): string[] {
	const issues: string[] = [];
	if (q.trackCode !== 'CPOP') return issues;
...
export function validateCpopDomainTags(rows: CpopQuestionRow[]): string[] {
	const issues: string[] = [];
	const domainCounts = zeroDomainCounts(CPOP_DOMAIN_TARGETS);
...
export function validateCpopTrack(rows: CpopQuestionRow[]): string[] {
	const issues: string[] = [];
	for (const q of rows) {
		issues.push(...validateCpopQuestion(q));
	}

	issues.push(...validateCpopDomainTags(rows));
