...
describe('CIS-TPRM v2 realism (full track)', () => {
	...
	it('passes shared realism validation', () => {
		expect(validateCisTprmTrack(proofBatch)).toEqual([]);
	});
	...
	it('tags each question with the official blueprint domain for its order', () => {
		const counts = zeroDomainCounts(CIS_TPRM_DOMAIN_TARGETS);
		for (const q of proofBatch) {
