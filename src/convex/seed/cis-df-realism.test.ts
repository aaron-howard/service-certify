...
import { CIS_DF_BANK_SIZE, CIS_DF_SCENARIO_MIN_RATIO, isScenarioStylePrompt, validateCisDfTrack } from '$lib/catalog/cisDfRealism';
...
describe('CIS-DF v2 realism', () => {
	...
	it('tags each question with a blueprint domain', () => {
		expect(rows.every((q) => Boolean(q.domain && q.domain.length > 0))).toBe(true);
	});

	it('tags each question with a content difficulty level', () => {
		expect(rows.every((q) => Boolean(q.contentDifficulty && q.contentDifficulty.length > 0))).toBe(
			true
		);
	});
});
