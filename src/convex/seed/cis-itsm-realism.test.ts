...
import {
	...
	validateCisItsmTrack
} from '$lib/catalog/cisItsmRealism';

const TRACK = 'CIS-ITSM';
...
describe('CIS-ITSM v2 realism (full track)', () => {
	...
	it('meets scenario-style minimum across the v2 bank', () => {
		...
		expect(scenarioCount / CIS_ITSM_BANK_SIZE).toBeGreaterThanOrEqual(0.65);
	});
	...
	it('uses direct answer choices without template prefixes', () => {
		for (const q of proofBatch) {
			for (const choice of q.choices) {
				for (const prefix of BANNED_CHOICE_PREFIXES) {
					expect(choice).not.toContain(prefix);
				}
			}
		}
	});
...
describe('CIS-ITSM full track realism', () => {
