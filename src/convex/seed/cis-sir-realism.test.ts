...
import {
	...
	validateCisSirTrack
} from '$lib/catalog/cisSirRealism';

const TRACK = 'CIS-SIR';
const V2_ORDERS = Array.from({ length: CIS_SIR_BANK_SIZE }, (_, i) => i);

describe('CIS-SIR v2 realism', () => {
	const rows = DEV_PRACTICE_QUESTIONS.filter((q) => q.trackCode === TRACK);

	it('has 90 v2 questions at orders 0-89', () => {
		expect(rows).toHaveLength(CIS_SIR_BANK_SIZE);
	...
	it('meets scenario-style minimum across the v2 bank', () => {
		...
		expect(scenarioCount / CIS_SIR_BANK_SIZE).toBeGreaterThanOrEqual(0.7);
	});
