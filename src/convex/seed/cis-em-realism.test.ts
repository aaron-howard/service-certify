...
import {
	type CisEmQuestionRow,
	BANNED_CHOICE_PREFIXES,
	BANNED_STEM_PREFIXES,
	CIS_EM_BANK_SIZE,
	CIS_EM_DOMAIN_TARGETS,
	STEM_OPENER_CAP,
	...
	validateCisEmTrack
} from '$lib/catalog/cisEmRealism';

const TRACK = 'CIS-EM';
const V2_ORDERS = Array.from({ length: 60 }, (_, i) => i);
...
describe('CIS-EM v2 realism (full track)', () => {
	const proofBatch = rowsFor(V2_ORDERS);

	it('has sixty v2 questions at orders 0-59', () => {
		expect(proofBatch).toHaveLength(60);
		expect(proofBatch.map((q) => q.order).sort((a, b) => a - b)).toEqual(V2_ORDERS);
	});

	it('passes shared realism validation', () => {
		expect(validateCisEmTrack(proofBatch as CisEmQuestionRow[])).toEqual([]);
	});
	...
	it('meets scenario-style minimum across the v2 bank', () => {
		...
		expect(scenarioCount / CIS_EM_BANK_SIZE).toBeGreaterThanOrEqual(0.65);
	});

	it('tags each question with the expected domain for its order', () => {
		const domainCounts = Object.fromEntries(
			Object.keys(CIS_EM_DOMAIN_TARGETS).map((d) => [d, 0])
		) as Record<string, number>;
		for (const q of proofBatch) {
			expect(q.domain).toBe(domainForOrder(q.order));
			domainCounts[q.domain!]++;
		}
		for (const [domain, target] of Object.entries(CIS_EM_DOMAIN_TARGETS)) {
