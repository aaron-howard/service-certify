...
- `question-gen/METHODOLOGY.md` — full methodology; **read it first**.
- `question-gen/tracks.config.json` — all 22 tracks: doc publications, exact domain names, target counts, keywords (mirrored from `trackDocSources.ts` + `examQuestionPolicy.ts`).
- `question-gen/validate.mjs` — structural gate. Run `npm run validate:questions` (or `node question-gen/validate.mjs <slug>`). Use `--seed` to validate `devQuestionBank.ts` directly.
- `question-gen/banks/<examSlug>.json` — one bank per exam slug (export with `npm run export:question-banks`).
- `src/convex/seed/devQuestionBank.ts` — live seed bank consumed by Convex (`npm run seed:dev:questions`).
- Domain names on questions MUST exactly match `domains[].name` in `tracks.config.json` / `src/lib/catalog/trackDocSources.ts`.

## Grounding source

[ServiceNow/ServiceNowDocs](https://github.com/ServiceNow/ServiceNowDocs) — official docs as markdown, branch per release family (`australia` default). Never scrape servicenow.com/docs (JS-only SPA).
...
1. Load the track’s config (publications, domains, weights, targetCount).
...
7. Fact-check ≥30% of keys against local docs (100% for cannot/must/only); cite `source` / `sourceUrls`.
...
- Original items only — never copy Pearson VUE, confidential exams, or brain dumps.
